"""
ml_system.py  —  ground-up redesign
===================================

What changed and why
--------------------
The previous ML layer had four structural problems that this redesign removes:

1. Train/serve skew + target leakage. Models were queried at inference with one
   feature distribution (agent EMA stats) and trained on another (per-call
   values, with the CSAT label leaking in as a feature). Fix: ONE `FeatureSchema`
   encoder is the single source of truth, used verbatim at decision time and at
   training time. The label is never an input.

2. Fabricated training data. Abandonment/FCR rows were synthesised with random
   draws because the simulation never recorded them. Fix: `DecisionLog` records
   the exact feature vector used for each decision and joins the *realised*
   outcome when the call finishes, so every (X, y) row is real.

3. Dead RL scaffolding. `RLStateBuilder`/`compute_reward` existed with no agent,
   policy, or update. Fix: a real contextual bandit (`LinUCBPolicy`) makes the
   routing decision and learns online from realised reward. The predictors feed
   it as *features* (predicted CSAT / resolution / abandonment), so the policy
   learns how to combine them — predictors as feature extractors, bandit as the
   policy.

4. No model evaluation + an orphaned forecaster. Fix: `ModelEvaluator` measures
   held-out AUC/Brier each epoch and *gates* retraining on validation gain; the
   `ArrivalForecaster` is now wired into the engine to drive a time-varying
   arrival rate.

The simulation side gains two pieces of realism that make the targets learnable:
stochastic patience (so abandonment is a genuine outcome, sampled from the
survival model) and a stochastic resolution outcome (so FCR is a real signal
rather than the input attribute `is_repeat`).

The module depends only on numpy (+ optional scipy/sklearn); it degrades to
closed-form fallbacks when the optional packages are missing.
"""
