...
# --- CP-SAT -> Gurobi -------------------------------------------------------
# OR-Tools (`from ortools.sat.python import cp_model`) is fully removed.
# gurobipy provides the model/solver; GRB carries status codes and var types.
...
class _StaffingModel:
    """Builds the staffing model one concern at a time (now on **Gurobi**).

    Pipeline (call in order) — unchanged from the CP-SAT version:
        add_decision_vars -> add_coverage_links -> add_base_floors
        -> add_total_agent_cap -> add_labour_constraints
        -> add_break_distribution -> set_objective

    CP-SAT -> Gurobi cheat-sheet used throughout this class:
        cp_model.CpModel()            -> gp.Model(env=...)
        model.NewIntVar(lb, ub, name) -> model.addVar(vtype=GRB.INTEGER, lb, ub, name)
        model.Add(expr <= rhs)        -> model.addConstr(expr <= rhs, name=...)
        model.Minimize(expr)          -> model.setObjective(expr, GRB.MINIMIZE)
        python sum(...)               -> gp.quicksum(...)   (best practice for LinExpr)
    Operator overloading (<=, >=, ==, +, -, * by constant) is supported by both
    APIs, so the algebraic constraint expressions are copied across verbatim.
    """
    ...
    def __init__(self, skills: List[str], shifts: List[str], opt_cfg: OptimizationConfig,
        ...
        # --- CP-SAT -> Gurobi: model + environment -------------------------
        # cp_model.CpModel() needed no environment. Gurobi requires a gp.Env; we
        # create one with OutputFlag suppressed (production-quiet) unless
        # debug_solver is set. Building the env empty + start() also hides the
        # license banner. The env is disposed in StaffingOptimizer.solve().
        self.env = gp.Env(empty=True)
        self.env.setParam("OutputFlag", 1 if opt_cfg.debug_solver else 0)
        self.env.start()
        self.model    = gp.Model("staffing", env=self.env)
        self.agents:  Dict[str, Dict[str, gp.Var]] = {}
        self.cover:   Dict[int, Dict[str, gp.Var]] = {}
    ...
    def add_decision_vars(self) -> "_StaffingModel":
        ...
        lb  = self.opt.min_agents_per_skill
        # CP-SAT: NewIntVar(0, cap) + a separate Add(>= min) constraint.
        # Gurobi: fold the lower bound straight into the variable (lb=min). This is
        # mathematically identical and yields one fewer constraint per variable.
        self.agents = {
            sh: {sk: self.model.addVar(vtype=GRB.INTEGER, lb=lb, ub=cap, name=f"n_{sh}_{sk}")
                 for sk in self.skills}
            for sh in self.shifts
        }
        self.model.update()   # make vars referencable before constraint construction
        return self
    ...
    def add_coverage_links(self) -> "_StaffingModel":
        ...
        for b, (_, _, active) in enumerate(COVERAGE_BANDS):
            self.cover[b] = {}
            for sk in self.skills:
                cv = self.model.addVar(vtype=GRB.INTEGER, lb=0, ub=cap, name=f"cover_b{b}_{sk}")
                # CP-SAT Add(cv == sum(...)) -> Gurobi addConstr with quicksum
                self.model.addConstr(
                    cv == gp.quicksum(self.agents[sh][sk] for sh in active),
                    name=f"coverlink_b{b}_{sk}")
                self.cover[b][sk] = cv
        self.model.update()
        return self
    ...
    def add_base_floors(self) -> "_StaffingModel":
        for b in range(len(COVERAGE_BANDS)):
            for sk in self.skills:
                self.model.addConstr(self.cover[b][sk] >= self.floors[(b, sk)],
                                     name=f"floor_b{b}_{sk}")
        # explicit, redundant peak-band floor (clarity / refactor safety)
        for sk in self.skills:
            self.model.addConstr(
                self.cover[PEAK_BAND_INDEX][sk] >= self.floors[(PEAK_BAND_INDEX, sk)],
                name=f"floor_peak_{sk}")
    ...
    def add_total_agent_cap(self) -> "_StaffingModel":
        if self.opt.max_total_agents is not None:
            self.model.addConstr(
                gp.quicksum(self.agents[sh][sk] for sh in self.shifts for sk in self.skills)
                <= int(self.opt.max_total_agents),
                name="total_agent_cap")
    ...
    def add_labour_constraints(self) -> "_StaffingModel":
            ...
            for b in lc.break_band_indices:
                if 0 <= b < n_bands:
                    for sk in skills:
                        ...
                        if buf > 0:
                            m.addConstr(self.cover[b][sk] >= self.floors[(b, sk)] + buf,
                                        name=f"breakbuf_b{b}_{sk}")
        ...
        if lc.max_total_budget is not None:
            ...
            m.addConstr(gp.quicksum(terms) <= int(round(lc.max_total_budget * _COST_SCALE)),
                        name="budget_cap")
            self.applied.append("budget_cap")

        # C. inter-shift smoothness  (|a1 - a0| <= swing encoded as two linear constraints)
        if lc.max_shift_swing is not None and len(self.shifts) >= 2:
            for sk in skills:
                for i in range(len(self.shifts) - 1):
                    ...
                    m.addConstr(a1 - a0 <= lc.max_shift_swing, name=f"swing_up_{sk}_{i}")
                    m.addConstr(a0 - a1 <= lc.max_shift_swing, name=f"swing_dn_{sk}_{i}")
            ...
            for b in range(n_bands):
                for (frm, to) in lc.cross_skill_pairs:
                    if frm in skills and to in skills:
                        ...
                        if floor > 0:
                            # auxiliary integer "credit" var (CP-SAT NewIntVar -> addVar)
                            credit = m.addVar(vtype=GRB.INTEGER, lb=0,
                                              ub=max(1, min(lc.cross_skill_cap, floor)),
                                              name=f"xcred_b{b}_{frm}_{to}")
                            m.addConstr(credit * 100 <= frac_num * self.cover[b][frm],
                                        name=f"xcred_cap_b{b}_{frm}_{to}")
                            m.addConstr(self.cover[b][to] + credit >= floor,
                                        name=f"xcred_floor_b{b}_{frm}_{to}")
        ...
        if lc.vip_uplift_scale > 0:
            for sk in skills:
                ...
                if uplift > 0:
                    m.addConstr(self.cover[PEAK_BAND_INDEX][sk] >= floor + uplift,
                                name=f"vip_uplift_{sk}")
        ...
        if lc.max_fte_per_skill:
            for sk, capf in lc.max_fte_per_skill.items():
                ...
                if terms:
                    m.addConstr(gp.quicksum(terms) <= int(capf), name=f"fte_cap_{sk}")
            ...
            for b in range(n_bands):
                for i in range(len(skills)):
                    for j in range(i + 1, len(skills)):
                        ...
                        m.addConstr(ci - cj <= lc.max_band_spread, name=f"spread_p_b{b}_{i}_{j}")
                        m.addConstr(cj - ci <= lc.max_band_spread, name=f"spread_n_b{b}_{i}_{j}")
        ...
        if lc.max_overstaff_ratio is not None:
            for b in range(n_bands):
                for sk in skills:
                    ...
                    m.addConstr(self.cover[b][sk] <= max(ceil_cov, self.floors[(b, sk)]),
                                name=f"overstaff_b{b}_{sk}")
        ...
        if lc.min_coverage_floor is not None:
            for b in range(n_bands):
                for sk in skills:
                    m.addConstr(self.cover[b][sk] >= int(lc.min_coverage_floor),
                                name=f"mincov_b{b}_{sk}")
    ...
    def add_break_distribution(self) -> "_StaffingModel":
        ...
        cap = self.opt.max_agents_per_skill
        # CP-SAT NewIntVar(0, cap) -> Gurobi addVar(vtype=GRB.INTEGER, ...)
        self.brk: Dict[str, Dict[str, Dict[int, gp.Var]]] = {}
        for sh in self.shifts:
            self.brk[sh] = {}
            for sk in self.skills:
                self.brk[sh][sk] = {
                    t: m.addVar(vtype=GRB.INTEGER, lb=0, ub=cap, name=f"brk_{sh}_{sk}_{t}")
        ...
        for sh in self.shifts:
            for sk in self.skills:
                ...
                if slots:
                    m.addConstr(gp.quicksum(slots.values()) == bpa * self.agents[sh][sk],
                                name=f"brk_demand_{sh}_{sk}")
                else:
        ...
        for t in range(n_slots):
            ...
            for sk in self.skills:
                ...
                present = gp.quicksum(self.agents[sh][sk] for sh in active_shifts)
                if on_break:
                    # (2) effective coverage >= Erlang floor for this slot's band
                    m.addConstr(present - gp.quicksum(on_break) >= self.floors[(band, sk)],
                                name=f"brk_cover_t{t}_{sk}")
                    # (3) at most `frac` of present agents on break simultaneously
                    m.addConstr(100 * gp.quicksum(on_break) <= frac100 * present,
                                name=f"brk_concurr_t{t}_{sk}")

        # (4) balance: minimise the busiest break slot (anti-concentration).
        # CP-SAT NewIntVar -> Gurobi addVar; the max() is encoded as break_peak >= each slot.
        self.break_peak = m.addVar(vtype=GRB.INTEGER, lb=0,
                                   ub=cap * len(self.shifts) * len(self.skills),
                                   name="break_peak")
        m.update()
        for t in range(n_slots):
            total_t = [self.brk[sh][sk][t] for sh in self.shifts for sk in self.skills
                       if t in self.brk[sh][sk]]
            if total_t:
                m.addConstr(self.break_peak >= gp.quicksum(total_t), name=f"brk_peak_t{t}")
            ...
            for sh in self.shifts:
                for sk in self.skills:
                    win = [v for t, v in self.brk[sh][sk].items()
                           if lo <= (t + 0.5) * L <= hi]
                    if win:
                        m.addConstr(gp.quicksum(win) >= self.agents[sh][sk],
                                    name=f"brk_lunch_{sh}_{sk}")
    ...
    def set_objective(self, obj_coeff: Dict[str, int]) -> "_StaffingModel":
        # NOTE: CP-SAT *required* an integer objective, so the original scaled every
        # £ coefficient by _COST_SCALE (=100). Gurobi accepts floating-point
        # objectives, but we KEEP the integer scaling unchanged so that downstream
        # numbers are byte-identical: ObjVal / _COST_SCALE still yields pounds, and
        # OptimizationResult fields match the CP-SAT build exactly.
        cost_term = gp.quicksum(obj_coeff[sk] * self.agents[sh][sk]
                                for sh in self.shifts for sk in self.skills)
        obj = cost_term
        ow = self.opt.overstaff_weight
        if ow > 0:
            overstaff = gp.quicksum(self.cover[b][sk] - self.floors[(b, sk)]
                                    for b in range(len(COVERAGE_BANDS)) for sk in self.skills)
            obj = obj + int(round(ow * _COST_SCALE)) * overstaff
        ...
        if bw > 0 and peak is not None:
            obj = obj + int(round(bw * _COST_SCALE)) * peak
            self.applied.append("break_balance")
        # CP-SAT model.Minimize(expr) -> Gurobi model.setObjective(expr, GRB.MINIMIZE)
...
class StaffingOptimizer:
    ...
    # --- CP-SAT -> Gurobi: status mapping -------------------------------
    # CP-SAT exposed OPTIMAL / FEASIBLE / INFEASIBLE / UNKNOWN / MODEL_INVALID.
    # Gurobi uses a larger set of GRB.* codes; we fold them onto the SAME strings
    # the rest of the pipeline already understands, so OptimizationResult.status
    # keeps its original vocabulary. TIME_LIMIT / INTERRUPTED are refined at
    # solve()-time using SolCount (incumbent present -> "feasible", else "unknown").
    _GUROBI_STATUS: Dict[int, str] = {
        GRB.OPTIMAL:     "optimal",
        GRB.SUBOPTIMAL:  "feasible",
        GRB.TIME_LIMIT:  "feasible",
        GRB.INTERRUPTED: "feasible",
        GRB.INFEASIBLE:  "infeasible",
        GRB.INF_OR_UNBD: "model_invalid",
        GRB.UNBOUNDED:   "model_invalid",
        GRB.NUMERIC:     "unknown",
    }
    ...
    def solve(self) -> OptimizationResult:
        ...
        # 1. analytical floors (with shrinkage gross-up)   [solver-agnostic, unchanged]
        band_erlang_min = _build_band_erlang_mins(
            self._sim, self._opt, self._target, lam_buffered, mean_svc)

        # 2. £ cost + SLA-penalty-adjusted objective coefficients   [unchanged]
        ...
        # 3. compose the model (one concern per call)   [identical pipeline, now Gurobi]
        ...
        # 4. solve
        # --- CP-SAT -> Gurobi: solver configuration -------------------------
        # CpSolver().parameters.*            ->  model.Params.*
        #   max_time_in_seconds              ->  TimeLimit
        #   num_search_workers = 8           ->  Threads = 8
        #   random_seed                      ->  Seed
        #   log_search_progress (debug)      ->  OutputFlag (set here + on the env)
        # Plus production-grade tuning Gurobi exposes that CP-SAT did not. The model
        # is small, so we prove optimality (MIPGap=0) to mirror CP-SAT's OPTIMAL
        # semantics; loosen MIPGap for very large instances if speed matters more.
        model.Params.TimeLimit = float(self._opt.cpsat_time_limit_seconds)
        model.Params.Threads   = 8
        if self._opt.random_seed is not None:
            model.Params.Seed = int(self._opt.random_seed)
        model.Params.MIPGap   = 0.0    # prove optimality (== CP-SAT "optimal")
        model.Params.MIPFocus = 2      # emphasise proving optimality
        model.Params.Presolve = 2      # aggressive presolve
        if self._opt.debug_solver:
            model.Params.OutputFlag = 1

        model.optimize()                                   # CP-SAT: solver.Solve(model)

        # --- CP-SAT -> Gurobi: status interpretation ------------------------
        gstatus    = model.Status
        status_str = self._GUROBI_STATUS.get(gstatus, f"gurobi_{gstatus}")
        # CP-SAT asked: status in (OPTIMAL, FEASIBLE)?  The robust Gurobi equivalent
        # is "does an incumbent exist?" -> SolCount > 0. This also rescues
        # TIME_LIMIT / INTERRUPTED runs that nonetheless found a feasible plan.
        has_solution = model.SolCount > 0
        if gstatus in (GRB.TIME_LIMIT, GRB.INTERRUPTED) and not has_solution:
            status_str = "unknown"

        # 5. extract plan (or Erlang fallback)
        if has_solution:
            # CP-SAT solver.Value(var) -> Gurobi var.X (a float); round to int.
            shift_plan = {sh: {sk: int(round(builder.agents[sh][sk].X)) for sk in skills}
                          for sh in shifts}
            ...
            obj_val = model.ObjVal                         # CP-SAT: solver.ObjectiveValue()
            wall_ms = model.Runtime * 1000.0               # CP-SAT: solver.WallTime() * 1000
            logger.info("Gurobi %s: plan=%s obj=£%.2f wall=%.1fms", status_str, shift_plan,
                        obj_val / _COST_SCALE, wall_ms)
        else:
            logger.warning("Gurobi %s — Erlang peak-band fallback.", status_str)
            shift_plan = {sh: {sk: band_erlang_min[(PEAK_BAND_INDEX, sk)] for sk in skills}
                          for sh in shifts}
            status_out = f"{status_str}_fallback"
            break_plan = {}

        # Read all decision values BEFORE freeing the model. Gurobi owns native
        # resources, so we explicitly dispose the model + env (no CP-SAT analogue).
        builder.model.dispose()
        builder.env.dispose()

        # 6. assemble result   [identical to the CP-SAT version]
