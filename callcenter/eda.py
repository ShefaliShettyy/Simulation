...
class EDALayer:
    ...
    def analyze(self, X: np.ndarray, y: np.ndarray,
        ...
        if self._ref_mean is not None and self._ref_mean.shape == means.shape:
            # For (near-)constant reference features, std ~ 0 would make any tiny
            # mean shift explode. Use the raw mean delta there (features are O(1)),
            # and cap per-feature drift so one feature can't dominate the score.
            denom = np.where(self._ref_std < 1e-6, 1.0, self._ref_std)
            shift = np.clip(np.abs(means - self._ref_mean) / denom, 0.0, 10.0)
